...
export default function StoryManagePage() {
  ...
  const handleViewItems = async (story: StoryData) => {
    ...
    try {
      ...
      const currentViews = Number(story.views) || 0;
      await storyService.incrementView(story.id, currentViews);

      // Update local state so UI reflects the new view count immediately
      setStories(prev => prev.map(s =>
        s.id === story.id ? { ...s, views: currentViews + 1 } : s
      ));
    } catch (error) {
